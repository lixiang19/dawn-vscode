...
const generateComponent = (componentName, fullPath, componentType) => {
  ...
  } else if (componentType === 'vueForm') {
    fcTemplate = path.resolve(__dirname, './file_template/vueForm.txt')
  } else if (componentType === 'vueTable') {
    fcTemplate = path.resolve(__dirname, './file_template/vueTable.txt')
  }
...
module.exports = function (context) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('extension.createNewVueForm', async (urlAll) => {
    // 文件夹绝对路径
    const folderPath = urlAll.fsPath
    const statPromise = util.promisify(fs.stat, fs)
    const stat = await statPromise(folderPath)
    if (stat.isFile()) {
      ...
      vscode.window.showInputBox(options).then(value => {
        ...
        const componentName = value
        const fullPath = `${folderPath}/${componentName}` + '.vue'
        generateComponent(componentName, fullPath, 'vueForm')
      })
    }
  }))
  context.subscriptions.push(vscode.commands.registerCommand('extension.createNewVueTable', async (urlAll) => {
    // 文件夹绝对路径
    const folderPath = urlAll.fsPath
    const statPromise = util.promisify(fs.stat, fs)
    const stat = await statPromise(folderPath)
    if (stat.isFile()) {
      ...
      vscode.window.showInputBox(options).then(value => {
        ...
        const componentName = value
        const fullPath = `${folderPath}/${componentName}` + '.vue'
        generateComponent(componentName, fullPath, 'vueTable')
      })
    }
  }))
}
